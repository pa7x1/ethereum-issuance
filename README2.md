...
given the risk premium, with a target stake ratio. Such that the stake ratio is not too low, which could lower economic security. 
And not too high, which could cause the protocol to overpay for security and push stakers to observe negative real yields at high stake ratios.
...
be able to match a stake ratio for any risk premium, however small it may be.
...
This is the risk premium the market demands for an LST. If the risk premium of an LST were to drop below 1.5%, we could face an ever-growing
stake ratio. In this regime the issuance curve fails to match the risk premium with a target stake ratio. We see 
this clearly if we flip the axis and treat the yield as the independent variable, which is what the market does.

![Risk Premium vs stake ratio](plots/figures/inverse_function.png)

The yield curve fails to match any stake ratio when the risk premium gets low enough, roughly below 1.5%, 
which would cause a runaway stake ratio. If this were to happen, 
...
can match the risk premium demanded by the market with a stake ratio.

The inverse of the issuance yield curve $y_i^{-1}$ must be a continuous invertible function that maps the 
issuance yield set by the market ($y_i = y_s - y_e$) to a stake ratio in the interval $(0, 1)$. 
That is, $y_i^{-1}: \mathbb{R} \to (0, 1)$ is a homeomorphism.

We impose continuity such that small changes in the risk premium do not cause abrupt changes in the stake ratio at which
...
strictly decreasing monotonicity by noting that the issuance yield must go up when the stake ratio goes to 0 
...
- The yield curve is able to match a stake ratio for any risk premium the market may demand.
- Continuity of the yield curve.
- Invertibility of the yield curve.
- High yields at low stake ratios. 
...
stake ratio for the given risk premium demanded by the market.
...
to adopt a specific form that implements stake capping. That is, provides 0 issuance yield at a certain target stake ratio.  
And, in the presence of exogenous yield, must contain a regime of negative issuance yields 
to avoid certain pathological regimes that cause unbounded stake ratio growth.
...
protocol to fix Ethereum's issuance curve and ensure it can match a healthy stake ratio for any risk premium.
...
> we could implement proper stake ratio capping. This approach would be preferred and provides for a more elegant design, 
...
a global factor to both we can set the desired yield at a particular stake ratio. The following curve has been tweaked
to have the following properties:

- The reward term has the same analytic form as today, $i_r(s) \sim \sqrt{s}$.
- The burn has a quadratic form, $i_b(s) \sim s^2$.
- Stake Capping at 50M ETH, slightly before 50% stake ratio.
- Yield at 25% stake ratio fixed at 3%. Same as today's current curve at 25 % ETH staked.
...
arguments that justify why the stake capping should be set before 50% stake ratio. We will address this from different perspectives;
the protocol point of view, the staker, and the holder. We will see that it is in the interest of all of them to keep
stake ratios under 50%.

**Protocol**

From the point of view of the protocol stake ratios above 50% start to become problematic. Above these levels, the majority 
...
From the point of view of a staker, stake ratios above 50% start to be self-dilutive and get stakers progressively close 
...
At very low stake ratios, the dilution effect of issuance is paid in full by holders which are the majority of the network.
At the other extreme, at stake ratios close to 100%, the issuance income is completely coming from self-dilution. At those
levels staking yield is not real income, it's a redenomination of the unit of account. Which, when accounting for expenses
and taxes, pushes validators to observe negative real yields. Very high stake ratios are bad for validators from a purely
economic perspective.

The point at which the transition between these 2 extremes happens is exactly at 50%. At stake ratios beyond that point
most of the issuance income is self-dilution. Hence, the self-interest of a staker is to introduce stake capping before
50% stake ratio to ensure staking ETH is guaranteed to provide a positive real yield, however low the risk premium may be.
...
and a target yield at a specific stake ratio. This means we have full freedom to set the issuance yield we want
at some target stake. Introducing the negative term does not need to imply a reduction in
the issuance yield at the target stake ratio.


In practice, healthy stake ratios are found in a range. Too close to 50% stake
rate is bad, but too close to 0% is also bad. A pragmatic middle point is 25%, which happens to be not
too far off current stake ratios (~30% circa Q4 2025).

Ethereum's issuance curve today provides a 3% issuance yield when 25% of ETH is staked. We could introduce a 
curve that maintains the same 3% issuance yield at 25% stake ratio but implements stake capping.  Doing so would solve once and for
all the threat of ever-growing stake ratios, but without disrupting the existing validator base.
...
would be slightly below 0.7%, a minor reduction with respect to today's 0.8%. But increases in the stake ratio would be met
...
below 1.5% we could face very high stake ratios.
...
- The slope towards negative yields is gentler than the example with quadratic burn. This results in the following:
  - Less exogenous yield could be burnt, up to 3%. Exogenous yields above that could cause runaway stake ratios.
...
but not 0%. If the risk premium of staking were to drop below that point it could face very high stake ratios.
- At around ~60M ETH, solo staking observes negative real yields which could cause solo stakers to get pushed out of the 
validator set.
- While LSTs remain viable until extremely high stake ratios.
- Two effects are at play that caused an increase of the gap between solo staking crossing 0% real yield and LSTs.
  - Tempered issuance has a very gentle slope of the real yield at high stake ratios. Gentler slopes tend to widen the gap. For contrast, check

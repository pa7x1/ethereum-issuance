...
real_staking_yield_solo = percentage_yield(real_issuance_yield(issuance_yield=vitaliks_issuance_yield(x),
                                                               supply=CIRCULATING_SUPPLY,
                                                               staked=x,
                                                               **home_staking))
real_staking_yield_lst = percentage_yield(real_issuance_yield(issuance_yield=vitaliks_issuance_yield(x),
                                                              supply=CIRCULATING_SUPPLY,
                                                              staked=x,
                                                              **lst_staking))
real_staking_yield_institutional = percentage_yield(
    real_issuance_yield(issuance_yield=vitaliks_issuance_yield(x),
                        supply=CIRCULATING_SUPPLY,
                        staked=x,
                        **institutional_staking))

real_holding_yield = percentage_yield(
    effective_holding_yield(issuance_yield=vitaliks_issuance_yield(x), supply=CIRCULATING_SUPPLY, staked=x))
...
plt.title("Vitalik's Stake Capping Real Issuance Yield")
...
plt.show()
fig.savefig('./vitalik_real_yield_plot.png', dpi=fig.dpi)